...
public class DialogBubbleReaderScript : MonoBehaviour {
	...
	public AudioClip soundToPlayOnFinish;
	...
	void loadSpeech() {
		...
		string dialogText =  speechPoints[speechPointIndex].speech[assignedSpeech].text;//Regex.Replace(speechPoints[speechPointIndex].speech[assignedSpeech].text, @"\s{2,}"," ");
		...
		if (startTalking) {
			...
			if (finishedTalking) {
				...
				if (nextObject) {
					...
					Camera.main.GetComponent<AudioSource>().clip = soundToPlayOnFinish;
